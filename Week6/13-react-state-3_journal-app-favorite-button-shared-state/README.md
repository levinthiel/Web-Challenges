...
x Accept a new prop in this component called `onToggleFavorite`. This event should be triggered when users press the favorite button.
x Accept a new prop called `isFavorite`.
x Accept a new prop called `id`.
x Remove the local state `isFavorite` from this component.
