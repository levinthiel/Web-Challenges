...
console.log("1 - " + maxOfTwoNumbers(5*5,9*2));
...
function averageWordLength(words) {
  const howManyWords = words2.length;
  let sumOfAllLength = 0;

  for (let i = 0; i < howManyWords; i++) {

    sumOfAllLength = sumOfAllLength + words2[i].length
  
  }

  const averageLength = sumOfAllLength / howManyWords;
  console.log("4 - " + averageLength)
}
...
function uniquifyArray(words3) {
  let newWords3 = []; 
  words3.forEach((word) => {
    if(!newWords3.includes(word)) {
      newWords3.push(word)
    } 
  })
  console.log("5 - " + newWords3)
}
...
function doesWordExist(haystack, needle) {
  if (haystack.find(word => word == needle) ) {
    console.log("6.1 - there's " + needle + " in the array")
  } else {
    console.log("6.1 - there's NO" + needle + "in the array")
  }

}
doesWordExist(wordsFind, "disobedience") 

//6.2 Count repetition  -----------
...
function howManyTimes(haystack, needle) {
  ...
  console.log("6.2 - The word " + needle + " appears " + timesneedle + " times in the array")
}
